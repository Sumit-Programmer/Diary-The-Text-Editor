...
main_application.title('Diary the text editor')
...
color_dict = {
    ...
    'Light Plus ': ('#4A545C', '#FFA4F9'),
    'Dark ': ( '#ECE9E4', '#403E3D'),
    'Red ': ('#5FFF3F', '#D52F3E'),
    'Monokai ': ('#1D2A70', '#705F44'),
    'Night Blue ': ('#FF90FC', '#0C1F76')
}
...
def changed(event=None):
    ...
    if text_editor.edit_modified():
        # Save the current state to history stack
        history_stack.append(text_editor.get('1.0', tk.END))
        text_changed = True
...
file.add_command(label='Save As', compound=tk.LEFT, accelerator='Ctrl+Alt+E', command=save_as)
...
# Add a new stack for redo actions
redo_stack = []
...
def undo(event=None):
    if history_stack:
        ...
        text_editor.delete('1.0', tk.END)
        text_editor.insert('1.0', history_stack[-3])

## Redo functionality
def redo(event=None):
    if redo_stack:
        history_stack.append(redo_stack.pop())
        text_editor.delete('1.0', tk.END)
        text_editor.insert('1.0', history_stack[-1])
